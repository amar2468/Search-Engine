...
The final method to discuss is the calculatePercentage() method which takes in the map entrySet() that contains the key,value of the number of occurrences of a term/terms in the file. Also, the int b variable is used to keep track of which file is being used. So if b = 0, we are talking about the first file. If b = 1, it is the second file and so on...
